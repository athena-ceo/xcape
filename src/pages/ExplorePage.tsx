...
export function ExplorePage() {
  ...
  // Excluded countries (filter violations) are always shown — they carry a "ne correspond pas"
  // warning inline, so there's no separate toggle to reveal them.
  const visible = useMemo(() => {
    if (!rows) return []
    const needle = q.trim().toLowerCase()
    const list = rows.filter((r) => !needle || placeName(r, lang).toLowerCase().includes(needle))
    return [...list].sort((a, b) =>
      sort === 'name' ? placeName(a, lang).localeCompare(placeName(b, lang)) : b.score - a.score)
  }, [rows, q, sort, lang])

  async function add(r: Row) {
    if (adding.has(r.place_id) || r.on_board) return
    const board = (rows ?? []).filter((x) => x.on_board)
      .map((x) => ({ place_id: x.place_id, name: placeName(x, lang), score: x.score }))
    setAdding((s) => new Set(s).add(r.place_id))
    try {
      const res = await addCountryToBoard(
        sid, { place_id: r.place_id, label: placeName(r, lang) }, board, t.explore.replaceConfirm)
      if (res === 'added') {
        // Refetch so on_board flags are authoritative (new in, evicted out) for the next add.
        const fresh = await api.explore(sid).catch(() => null)
